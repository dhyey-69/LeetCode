...
def xyz(prices,strategy,k):
    ...
    for i in range(n - k + 1):
        first_loss = pref_profit[i + half] - pref_profit[i]

        second_orig = pref_profit[i + k] - pref_profit[i + half]
        second_new = pref_prices[i + k] - pref_prices[i + half]

        gain = -first_loss + (second_new - second_orig)
